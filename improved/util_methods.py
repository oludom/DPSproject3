...
def get_top_node(node_details, node_id):
    higher_node_array = []
    for each in node_details:
        if each['node_id'] > node_id:
            higher_node_array.append(each['port'])
    return None if len(higher_node_array) == 0 else max(higher_node_array)


# this method is used to send the higher node id to the proxy
def elect_top_node(top_node, node_id):
    status_code_array = []
    url = 'http://localhost:%s/proxy' % top_node
    data = {
        "node_id": node_id
    }
